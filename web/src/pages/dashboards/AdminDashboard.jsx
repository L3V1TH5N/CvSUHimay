// file path: web/src/pages/dashboards/AdminDashboard.jsx
...
const useSidebarPref = () => {
  const [collapsed, setCollapsed] = useState(() => {
    try { return localStorage.getItem('cvsuhimay.admin.sidebar.collapsed') === 'true'; }
    catch { return false; }
  });
  const toggleCollapsed = () => {
    setCollapsed(prev => {
      const next = !prev;
      try { localStorage.setItem('cvsuhimay.admin.sidebar.collapsed', String(next)); }
...
const SidebarUserBlock = ({ user, collapsed }) => {
  const displayName = user?.full_name?.split(' ')[0] || user?.email || 'Admin';
